...
if __name__ == '__main__':
    ...
    matches_file=open(outputfolder+dir+"/matches.csv",'w')
    ...
    while True:
            ...
            try:
                 ...
                 print "Frame with no features, skipped"    
                 continue  
    ...
    features_txt_file.write("Exec time: {0} sec \per frame: {1} sec\n".format( total_time,total_time/FRAME_COUNT))
...
+''.join([' %s: %s \n' % (key, value) for (key, value) in props.items()]))
    ...
    for t,matched,alldetected in zip(sampling_times,matched_features,number_of_features):
          matches_file.write(str(t)+",\t" +str(matched)+",\t" +str(alldetected)+"\n")
    matches_file.close()
